...
Command Palette ← WinRT/COM → SysMonCmdPal.exe (.NET 10)
                                   ...
                                   ├── Commands/               (DockFormat + 刷新协调器 + 7 种 DockBand)
                                   ├── Pages/                  (详情页 + 共享刷新基类)
                                   ├── SystemInfoService       (1s 聚合刷新 + SystemSnapshot + 图表推进)
                                   ├── Broker/                 (共享内存读取 + 快照管理)
                                   ...
                                   ├── Services/
                                   │   ├── CpuUsageReader      (CPU PerformanceCounter 生命周期)
                                   │   ├── SystemMemoryReader  (GlobalMemoryStatusEx)
                                   │   ├── SystemBatteryReader (GetSystemPowerStatus + 状态判定)
                                   │   ├── CpuSensorReader     (CPU 温度回退链)
                                   │   ├── GpuSensorReader     (GPU 数据回退链)
                                   │   ├── DiskMonitor / NetworkMonitor
                                   │   └── SparklineChart      (PNG/SVG 火花线渲染)
...
`settings.json` 中旧的 `PrecisionMode` 字段仍可读取和原样写回，但当前不提供 CmdPal 手动切换入口，也不会覆盖这条自动回退链。
...
```
sysmon-cmdpal/
├── SysMonCmdPal.sln                    # 当前包含主扩展、Broker、Tests、LhmTest
...
│   │   ├── CpuDockBand.cs                # CPU Dock Band
│   │   ├── MemoryDockBand.cs             # 内存 Dock Band
│   │   ├── DiskDockBand.cs               # 磁盘 Dock Band
│   │   ├── NetworkDockBands.cs           # 下载/上传 Dock Band
│   │   ├── BatteryDockBand.cs            # 电池 Dock Band
│   │   ├── GpuDockBand.cs                # GPU Dock Band
│   │   └── BtopLauncherCommand.cs        # btop4win 一键启动
│   ├── Pages/
│   │   ├── RefreshingContentPage.cs      # FormContent 详情页共享刷新订阅基类
...
│   ├── Services/
│   │   ├── SystemInfoService.cs      # 系统数据聚合器 (1s Refresh, 快照/图表推进)
...
│   │   ├── SystemBatteryReader.cs    # GetSystemPowerStatus 电池读取
