...
```
Broker 共享内存 v2 (LHM, 管理员) ──✓──> 全功能 (CPU + GPU + 全量传感器)
        │ ✗ 或数据过期 (>10s)
        ├── HWiNFO 共享内存 ──✓──> CPU + GPU 温度 (用户态, 每 ~12h 需重启)
        │                │ ✗
        │                └── 不可用 → UI 显示后端状态
        │
...
Broker 不可用时自动降级到 HWiNFO，再降级到 ThermalZone。UI 实时显示当前数据源。
...
| 网速 | `NetworkInterface.GetAllNetworkInterfaces()` delta (仅物理接口, EMA 平滑) |
| 电池 | `GetSystemPowerStatus` P/Invoke |
| CPU 温度 | **Tier 1**: Broker SHM → **Tier 2**: HWiNFO SHM → **Tier 3**: ThermalZone |
| GPU 温度 | **Tier 1**: Broker SHM → **Tier 2**: HWiNFO SHM → **Tier 3**: ThermalZone |
